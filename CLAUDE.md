...
Seven `ResourceDictionary` XAML files live in `PhotoPresenter/Themes/`: `Light.xaml`, `Dark.xaml`, `HighContrastLight.xaml`, `HighContrastDark.xaml`, `LightLargeText.xaml`, `DarkLargeText.xaml`, `SlateBlue.xaml`. Each defines ten named `SolidColorBrush` resources (`AppBackground`, `PanelBackground`, `PanelText`, `SplitterBackground`, `ListBackground`, `ThumbnailBackground`, `VideoOverlayBackground`, `VideoIconForeground`, `FilenameForeground`, `CaptionForeground`), four `SystemColors` key overrides for ListBox selection, and eight `sys:Double` font-size resources (`FontSize.XSmall/Small/Medium/Normal/Large`, `FontSize.PresentCaption/PresentLoading/VideoIcon`). Large-Text variants scale all sizes by ~130%; color-only themes (Dark, HC variants, SlateBlue) share normal sizes. `SlateBlue.xaml` additionally overrides `SystemColors.MenuBrushKey`, `MenuTextBrushKey`, `MenuHighlightBrushKey`, and `MenuBarBrushKey` so context menus adopt the theme palette.
