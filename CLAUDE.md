...
| Services | `Services/` | `PhotoLibraryService` — scan folders, apply/save sidecar ordering; `UserSettings` — persist last folder, selected folder, window bounds, splitter position, and theme to `%APPDATA%\PhotoPresenter\settings.json`; `ThemeService` — swap the active theme ResourceDictionary at runtime |
...
`UserSettings` (`Services/UserSettings.cs`) stores: `LastParentFolder`, `LastSelectedFolder` (folder name), `LastSelectedPhoto` (filename), `PhotoScrollOffset`, `FolderScrollOffset`, `ShowAllFolders`, `ShowAllPhotos`, `Volume` (Present mode, default 0.5), window bounds, `WindowMaximized`, `SplitterPosition`, and `Theme` (default `"Light"`). All names are matched case-insensitively on restore. `Window_Closing` in `MainWindow.xaml.cs` is the authoritative save point for most settings — it reloads the file first to pick up any mid-session saves (e.g. splitter drags), then adds window bounds, `LastSelectedFolder`, `LastSelectedPhoto`, and `Theme` before writing. On startup, `MainViewModel` passes both saved names to `OrganiseViewModel.LoadAsync`, which restores the folder first (falling back to the first folder if not found), then restores the photo within that folder's active `Photos` collection (falling back to no selection if not found). `OnSelectedFolderChanged` resets `SelectedPhoto` to null synchronously, so the photo assignment in `LoadAsync` runs after that reset. `ShowAllFolders` and `ShowAllPhotos` are set on `OrganiseVM` immediately after firing `LoadAsync` (which suspends at its first `await`), so the flags are in place before the collections are populated on resume.
...
Eight `ResourceDictionary` XAML files live in `PhotoPresenter/Themes/`: `Light.xaml`, `Dark.xaml`, `HighContrastLight.xaml`, `HighContrastDark.xaml`, `LightLargeText.xaml`, `DarkLargeText.xaml`, `SlateBlue.xaml`, `SlateBlueLargeText.xaml`. Each defines ten named `SolidColorBrush` resources (`AppBackground`, `PanelBackground`, `PanelText`, `SplitterBackground`, `ListBackground`, `ThumbnailBackground`, `VideoOverlayBackground`, `VideoIconForeground`, `FilenameForeground`, `CaptionForeground`), four `SystemColors` key overrides for ListBox selection, and eight `sys:Double` font-size resources (`FontSize.XSmall/Small/Medium/Normal/Large`, `FontSize.PresentCaption/PresentLoading/VideoIcon`). Large-Text variants scale all sizes by ~130%; color-only themes (Dark, HC variants, SlateBlue) share normal sizes. The Slate Blue pair additionally overrides `SystemColors.MenuBrushKey`, `MenuTextBrushKey`, `MenuHighlightBrushKey`, and `MenuBarBrushKey` so context menus adopt the theme palette.

`App.xaml` wraps its `DataTemplate` resources inside a top-level `ResourceDictionary` with `Light.xaml` as `MergedDictionaries[0]`. `App.OnStartup` (`App.xaml.cs`) calls `ThemeService.Apply(savedTheme)` before the window appears, so there is no flash. `ThemeService.Apply` (`Services/ThemeService.cs`) replaces `MergedDictionaries[0]` with a new `ResourceDictionary` loaded from the named theme file — all `DynamicResource` bindings in `OrganiseView.xaml` and `PresentView.xaml` update immediately. `OrganiseView.xaml` binds all colors and font sizes via `DynamicResource`; `PresentView.xaml` binds only font sizes (Present mode always has a black backdrop). Both `FolderList` and `PhotoList` set `Background="{DynamicResource ListBackground}"` explicitly — without this the ListBox renders with `SystemColors.WindowBrush` (white) regardless of theme. Folder name TextBlocks carry an explicit `Foreground="{DynamicResource FilenameForeground}"` for the same reason. The theme `ComboBox` in `MainWindow.xaml` toolbar wires its `SelectionChanged` event in code-behind after `InitThemeComboBox()` sets the initial item, preventing a spurious save on startup.
